...
One opencode session per honeybee: `HONEYBEE.md` (read from the repo root) as system prompt, first user prompt = bootstrap/reconcile/select/review/arbitrate context. cwd = beehive repo root. After each turn the runner checks completion deterministically:
