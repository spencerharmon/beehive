...
| `RaiseConflict` | action | a merge of two incomparable `main` forks that git cannot auto-merge — `swarm.go:421 resolveConflict` hands the agent the conflicted files; a gitlink (mode 160000) conflict "cannot be resolved by editing" (`swarm.go:~440`) | exercised by all three `MainConvergence_conflict*` cfgs |
| `ResolveConflict` | action | the agent's resolution attempt inside `resolveConflict`/`publishWithResolution` (`swarm.go:426`); honest → the UNION of both sides, byzantine → no progress (the re-handed conflict loop). The runner bounds attempts (`MergeRetries` / `MaxResolveAttempts`) | `swarm_test.go:3492 TestPublishWithResolutionResolvesConflict`, `:3514 TestPublishWithResolutionDefersWhenUnresolved` |
| `PublishMerged` | action | committing the resolved (union) merge back to the tracked branch / both anchors (`publishWithResolution` retry publish) | `swarm_test.go:3492 TestPublishWithResolutionResolvesConflict`; `MainConvergence_conflict_fixed.cfg` |
| `PublishMergedLossy` (buggy) | action | a byzantine "pick one side" resolution of an unresolvable gitlink conflict that commits a merge dropping the other side's artifacts | reproduced by `MainConvergence_conflictloss_buggy.cfg` |
| `ConflictTerminates` | invariant | the runner bounds conflict-resolution attempts (`MergeRetries`) then defers rather than looping — an unresolved conflict aborts to a clean worktree and a deferred `ErrConflict`, never an unbounded loop | `swarm_test.go:3514 TestPublishWithResolutionDefersWhenUnresolved` (bounded, defers); `MainConvergence_conflictloop_buggy.cfg` reproduces the unbounded loop; `MainConvergence_conflict_fixed.cfg` holds |
| `NoMergeLoss` | invariant | `NoSilentLoss` at MERGE granularity — a published conflict resolution must be a superset of the union of both conflicting sides, never drop a side; the reviewer/arbiter re-check gate is the code guard against a byzantine dropped-side merge | `MainConvergence_conflictloss_buggy.cfg` reproduces the dropped-side loss; `MainConvergence_conflict_fixed.cfg` holds |
...
| `NoFalseDone` (durable ∧ merged ∧ content-present ∧ DoD-check) | invariant | `verify.go verifyGate` (durability + invariant-5 check gate) + `finalizeIfAlreadyMerged`/`recordReviewedCommit` own-tip fix | `TaskStatus_buggy.cfg` (durability defect) + `TaskStatus_buggy_check.cfg` (jellyfin DoD defect) both prove false-DONE reachable when the respective gate is off |
| `FinalizeAlreadyMerged` content gate (`ContentFinalize`) + `RevertMerge`/`MergeLandsInterrupted`/`ReopenReverted` + `contentPresent` in `NoFalseDone` | action + guard + invariant | `swarm.go:2651 finalizeIfAlreadyMerged` — **GAP (flux:omada-ingress-tls false-DONE 2026-08-02): finalizes DONE from merge ANCESTRY, not from the reviewed tree CONTENT surviving on `main`, and skips the `Check:`** ("branch not required"). The Go fix (`review-revert-content-finalize-fix`, depends on this spec) must gate finalize on content-present + `Check:` | `TaskStatus_revertfinalize_buggy.cfg` (`ContentFinalize=FALSE`, ancestry-only finalize) reproduces `NoFalseDone` violated — a DONE state with `merged=TRUE ∧ contentPresent=FALSE` reached via `MergeLandsInterrupted → RevertMerge → FinalizeAlreadyMerged`; `TaskStatus_revertfinalize_fixed.cfg` (`ContentFinalize=TRUE`) holds `NoFalseDone`/`Terminates` across the revert trace (reverted merges re-open via `ReopenReverted`) |
