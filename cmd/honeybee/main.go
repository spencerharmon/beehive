...
func run() error {
	...
	// single flat, host-level knob — never resolved per-submodule. A pass keeps its
	// own claim fresh via swarm.Runner's mid-turn heartbeat keepalive (~ttl/3), so a
	// claim with no heartbeat for a full ttl is genuinely dead; racePeerOwnsTask is
	// the authoritative backstop that detects a race that slips past it.
