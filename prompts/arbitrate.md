...
settle the dispute between the implementer and the reviewer. **Do NOT reimplement the task, do NOT
...
- **SIDE WITH THE REVIEWER**: the rejection stands. Set the PLAN.md task -> TODO (or NEEDS-HUMAN once
  past the attempt limit) and record the binding rationale in the task body / a doc so the next
  implementer knows what to fix. Commit.
