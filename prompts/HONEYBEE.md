...
**This file is the authoritative honeybee protocol. The runner reads it from the beehive repo root
at the start of every pass and injects it as your system prompt** — it is NOT hard-coded in the
binary, so the operator can edit it. The binary ships a default copy; `beehive instruction update`
refreshes it (with backup) when the tool's protocol advances. If this file and a per-repo `AGENTS.md`
or `LOCALS.md` ever disagree about the *protocol*, this file wins; site facts (paths, hosts, deploy)
live in `LOCALS.md` and are authoritative for those.
