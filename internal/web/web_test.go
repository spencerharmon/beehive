...
func TestScrollPreserveWiring(t *testing.T) {
	...
	// The body now renders as structured turns (session-transcript-rendered-toc),
...
// session-transcript-rendered-toc: the poll fragment renders the flat transcript
// as structured, sanitized turn <section>s (markdown -> HTML, NOT a raw dump)
// and ships a TOC overlay whose jump links/buttons target each turn's anchor —
// all WITHOUT dropping poll-scroll-preserve's stable id + scroll-preserve/pin
// contract on the scroll container.
