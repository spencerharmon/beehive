...
//  4. bootstrap_agent.html / chatedit_panel.html apply the same idle backoff to
//     the setup wizard's chat panel — the last polled chat/diff surface still
//     on an unconditional interval (bootstrap-chat-poll-backoff). The wizard is
//     chatedit_panel.html's only caller now (web.go's /edit/{id}/* family), so
//     this closes the one panel poll-backoff-trigger-landing missed.
func TestPollBackoffWhenEndedOrIdle(t *testing.T) {
	...
	// (4) bootstrap chat: the wizard's chat shell polls once; chatedit_panel.html
	// self-perpetuates only while Busy — the same backoff, now on the last polled
	// chat/diff panel that still lacked it (bootstrap-chat-poll-backoff).
	bShell := renderTmpl(t, s, "bootstrap_agent.html", map[string]interface{}{"ID": "b1"})
	if !strings.Contains(bShell, `hx-trigger="load"`) || strings.Contains(bShell, "every") {
		t.Errorf("bootstrap_agent.html shell must poll once on load, not on an interval:\n%s", bShell)
	}
	bIdle := renderTmpl(t, s, "chatedit_panel.html", map[string]interface{}{"ID": "b1", "Busy": false})
	if strings.Contains(bIdle, "hx-trigger") {
		t.Errorf("idle chatedit_panel must carry NO poller (nothing to watch):\n%s", bIdle)
	}
	bBusy := renderTmpl(t, s, "chatedit_panel.html", map[string]interface{}{"ID": "b1", "Busy": true})
	...
	// Only the cadence changes: the idle and busy panels swap the same target.
	if !strings.Contains(bBusy, `hx-swap="innerHTML"`) {
		t.Errorf("busy chatedit_panel re-arm must keep the innerHTML swap:\n%s", bBusy)
	}
}
...
// cell whose full value stays reachable via the code's title (so a long sha
// never overflows the card). It reuses the setup fixture (alpha, stamp abc123)
// and drives the idle vs live-bee states via the on-disk claim heartbeat (real
// time.Now()).
...
// the always-present top-nav bar (which also links "/" and "/stats"). Empty means
// the page rendered no breadcrumb.
...
func TestDocExplorerListsWholeTree(t *testing.T) {
	...
	// Each listing link threads ?from=docs so the doc viewer's breadcrumb names
	// the docs explorer as the entry route.
...
func TestNavAriaCurrentOptional(t *testing.T) {
	...
	// A submodule-scoped page is not one of the 7 top-nav routes, so no top-nav
	// link lights up. The breadcrumb-trail-landing trail, however, legitimately
	// carries aria-current on its OWN terminal crumb (a distinct
	// <nav aria-label="breadcrumb"> landmark whose leaf is a <span>, never a
	// top-nav <a>), so scope this "nothing current" check to the top-nav rather
	// than the whole page — the two aria-currents coexist on different elements.
	...
	if strings.Contains(topnav, `aria-current="page"`) {
		t.Fatalf("plan page should not mark any top-nav link current:\n%s", plan)
	}
	// Coexistence guard: the breadcrumb landmark's own current crumb IS present
	// (on a <span>, outside the top-nav), proving the two aria-currents never
