...
// the per-submodule branches view. A repo commit sha is a SUBMODULE-scoped object,
// not a hive commitView target (linking it would 404), so the sha stays plain,
// selectable <code> text (the no-JS fallback) and instead gains a progressively-
// enhanced .copy-btn carrying the sha in data-copy plus an accessible label. The
// sha itself must never become a dead deep link.
...
// hive commit page, so the heading sha is the page's own identity, not a link.
...
// TestCopyClipboardScriptEmbedded locks that the commit-sha-deep-links copy script
// and its shared aria-live region ship on a real full-page response (the layout
// footer), progressively enhanced and dependency-free. It is what turns every
// [data-copy] sha into a one-click copy without leaving a dead control when JS is
// off. Mirrors TestScrollPreserveScriptEmbedded.
...
// legitimate deep link (the delivery-traceability wiring this task builds on): the
// /stats deliveries column renders the hive flip sha as the CLICKABLE anchor text
// of a link to that commit's commitView — the visible sha is itself the deep link,
// not a separate label. (Submodule shas, by contrast, get the copy control above
// because they have no hive commitView target.)
