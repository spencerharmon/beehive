...
// TestSessionStreamFollowsOffBoxAndBannersSync proves the re-cut's key fix: the
// SSE stream path itself FOLLOWS off-box (remote-host) sessions and surfaces the
// staleness banner. The pre-integration branch had exactly this gap — its
// sessionStream looped a transcript read that never fast-forwarded local main, so
// a stub an agent published on ANOTHER host streamed "(waiting…)" forever, and it
// bypassed the .Sync banner while the live page cancelled the htmx poll. Here a
// producer on "another host" publishes a session stub on origin/main plus its
// transcript on an isolated stream branch; the local viewer starts behind.
// Connecting to /stream must (1) fast-forward local main so the stub lands on
// disk, (2) resolve the stream branch and push the off-box transcript as a data:
// frame, and (3) push the follow staleness banner as an `event: sync` (clean
// fast-forward => no stall). A real server is used because the stream is held
// open; the handler must also return promptly on client disconnect (no leak).
func TestSessionStreamFollowsOffBoxAndBannersSync(t *testing.T) {
	t.Setenv("GIT_ALLOW_PROTOCOL", "file") // clone/push over local file paths
	s, root := setup(t)
	s.pullIvl = 0                            // every stream tick actually pulls (determinism)
	s.streamInterval = 20 * time.Millisecond // re-read fast so the test stays quick
	...
	// Seed local main and normalize the branch name (setup only `git init`ed).
	writeAt(root, "seed", "x")
	gitAt(t, root, "add", "-A")
	gitAt(t, root, "commit", "-q", "-m", "seed")
	gitAt(t, root, "branch", "-M", "main")

	// Bare origin = the shared remote both hosts converge on; publish local main.
	origin := filepath.Join(t.TempDir(), "origin.git")
	gitAt(t, filepath.Dir(origin), "init", "-q", "--bare", "-b", "main", "origin.git")
	gitAt(t, root, "remote", "add", "origin", origin)
	gitAt(t, root, "push", "-q", "origin", "main")

	sessRel := "submodules/alpha/sessions/bee-remote.md"
	localSess := filepath.Join(root, filepath.FromSlash(sessRel))

	// Off-box honeybee (clone on "another host"): transcript on the stream branch,
	// stub on main — exactly what the streaming producer does — pushed to origin.
	work := t.TempDir()
	gitAt(t, work, "clone", "-q", origin, "producer")
	prod := filepath.Join(work, "producer")
	gitAt(t, prod, "config", "user.email", "b@b")
	gitAt(t, prod, "config", "user.name", "bee")
	gitAt(t, prod, "checkout", "-q", "-b", "bee-remote-stream")
	writeAt(prod, sessRel, "# session bee-remote\n\nhello from off-box\n")
	gitAt(t, prod, "add", "-A")
	gitAt(t, prod, "commit", "-q", "-m", "stream turn 1")
	gitAt(t, prod, "push", "-q", "origin", "bee-remote-stream")
	gitAt(t, prod, "checkout", "-q", "main")
	writeAt(prod, sessRel, repo.SessionStub("bee-remote-stream"))
	gitAt(t, prod, "add", "-A")
	gitAt(t, prod, "commit", "-q", "-m", "publish session stub")
	gitAt(t, prod, "push", "-q", "origin", "main")

	// Before any request the off-box session is invisible here (local main behind).
	...
	// Wrap the handler so the test observes when it RETURNS (leak guard), then
	// connect a real streaming client (the stream is held open).
	handlerDone := make(chan struct{})
	mux := http.NewServeMux()
	mux.HandleFunc("GET /submodule/{name}/session/{branch}/stream", func(w http.ResponseWriter, r *http.Request) {
		defer close(handlerDone)
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, "GET", srv.URL+"/submodule/alpha/session/bee-remote/stream", nil)
	if err != nil {
		t.Fatal(err)
	}
	resp, err := http.DefaultClient.Do(req)
	...
	// Read frames off the wire in the background so every wait is bounded.
	lines := make(chan string, 512)
	go func() {
		br := bufio.NewReader(resp.Body)
		for {
	...
	// waitAll blocks until every substring has been seen across the stream (each
	// may land on a different frame line), or the deadline passes.
	seen := map[string]bool{}
	waitAll := func(subs ...string) {
		t.Helper()
		deadline := time.After(3 * time.Second)
		for {
			all := true
			...
			case ln, ok := <-lines:
				if !ok {
					t.Fatalf("stream closed before seeing all of %v; seen=%v", subs, seen)
				}
				for _, sub := range subs {
			...
			case <-deadline:
				t.Fatalf("timed out waiting for %v; seen=%v", subs, seen)
			}
		}
	}
	...
	if _, err := os.Stat(localSess); err != nil {
		t.Fatalf("stream did not fast-forward the off-box stub onto local main: %v", err)
	}

	// Leak guard: disconnecting must make the SERVER handler return promptly.
	cancel()
	select {
	case <-handlerDone:
	case <-time.After(3 * time.Second):
