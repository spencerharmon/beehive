...
// Crumb is one node in a page's breadcrumb trail (breadcrumb-trail-landing). Href
// is the link target for an ANCESTOR crumb; the terminal (current-page) crumb has
// an empty Href and the breadcrumb partial (layout.html) renders it as
// aria-current text rather than a link. Every builder below therefore leaves
// ONLY the last crumb's Href empty, so "empty Href == current page" is an
// invariant the template can key off without a separate flag.
type Crumb struct {
	Label string
	Href  string
}
