...
func TestRunCompletes(t *testing.T) {
	...
	os.WriteFile(planPath, []byte("- T1 | IN-PROGRESS | go | ts:2026-06-29T10:00:00Z\n"), 0o644)
	...
	cl := &mockClient{sess: &mockSession{onTurn: func(turn int) {
		...
		os.WriteFile(planPath, []byte("- T1 | DONE | go\n"), 0o644)
	}}}
	r.Client = cl
...
func TestRunGCCap(t *testing.T) {
	...
	os.WriteFile(filepath.Join(sm, "PLAN.md"), []byte("- T1 | IN-PROGRESS | go | ts:2026-06-29T10:00:00Z\n"), 0o644)
	g.Commit(context.Background(), "seed")
	rp, _ := repo.Open(root)
	subs, _ := rp.Submodules()
	sel := &selectt.Selection{Kind: selectt.Work, Submodule: subs[0], Task: plan.Task{ID: "T1"}}
