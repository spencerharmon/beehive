...
func (r *Runner) Run(ctx context.Context, sel *selectt.Selection, system, first string) (Result, error) {
	...
	switch sel.Kind {
	case selectt.Review:
		// A doc-only task (its Files: line touches only beehive-layer text — docs/,
		// PLAN.md, ROI.md, docs/tasks/) has NO bee-<taskid> code branch, so telling the
		// reviewer to fetch/inspect it just earns a "couldn't find remote ref" failure
		// before the doc fallback. Classify from the card and state the fact instead.
		var branchSentence string
		if isDocOnlyCard(sel.Task.Body) {
			branchSentence = fmt.Sprintf(
				"This is a DOC-ONLY task: no bee-%[2]s code branch exists (nothing to fetch/inspect) — "+
					"judge the change doc submodules/%[1]s/docs/bee-%[2]s-%[2]s.md and PLAN.md state directly.\n",
				smName, sel.Task.ID)
		} else {
			branchSentence = fmt.Sprintf(
		...
		preamble = fmt.Sprintf(
			...
			smName, sel.Task.ID, branchSentence)
	case selectt.Arbitrate:
		var branchSentence string
		if isDocOnlyCard(sel.Task.Body) {
			branchSentence = fmt.Sprintf(
				"This is a DOC-ONLY task: no bee-%[2]s code branch exists (nothing to fetch/inspect) — "+
					"judge the change doc submodules/%[1]s/docs/bee-%[2]s-%[2]s.md and the reviewer rejection doc "+
					"submodules/%[1]s/docs/%[2]s-review-reject.md directly.\n",
				smName, sel.Task.ID)
		} else {
			branchSentence = fmt.Sprintf(
